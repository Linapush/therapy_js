...
// import createClient from '../db_connection.js';
...
router.post('/login', async (req, res) => {
    const { username, password } = req.body;
    try {
        console.log('Executing query:', 'SELECT * FROM users WHERE username = $1', [username]);
...
router.post('/admin/login', async (req, res) => {
    ...
    try {
        ...
        console.log('Before setting session user:', user);
        req.session.user = {
            id: user.id,
            username: user.username,
            role: user.role
        ...
        console.log('Куки с токеном установлены:', res.getHeader('Set-Cookie'));
